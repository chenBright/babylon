...
class CompactEnumerableThreadLocal {
 ...
 private:
  ...
  static uint32_t allocate_id() noexcept {
    BABYLON_LEAK_CHECK_DISABLER;
    return id_allocator().allocate().value;
  }

  template <bool L = Leaky>
  static typename std::enable_if<L, IdAllocator<uint32_t>&>::type id_allocator() noexcept {
...
#pragma GCC diagnostic pop
  ...
  template <bool L = Leaky>
  static typename std::enable_if<L, Storage&>::type storage(size_t slot_index) noexcept {
#pragma GCC diagnostic push
#pragma GCC diagnostic ignored "-Wpragmas"
#pragma GCC diagnostic ignored "-Wunknown-warning-option"
#pragma GCC diagnostic ignored "-Wexit-time-destructors"
